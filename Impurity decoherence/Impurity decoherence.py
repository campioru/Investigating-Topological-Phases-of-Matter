...
del (LxLy, LxLy2, SM, SMeiDt, Mdage_iEt, h0, h0_vals, Vdag, e_iE_diag, h1,
     h1_vals, M, eiD_diag, Mdag, I_S_diag, final)
...
for D, D_ in enumerate(Deltas):
    ...
    for m in range(m_l):
        for p in range(p_l):
            for T in range(T_l):
                i, j = p + m*p_l, T
                ax[i, j].plot(ts_, nus_[0, L, m, T, mu, p, D, 0],
                              color=colours[j])
                ax[i, j].relim()
                ax[i, j].autoscale(axis="y")
...
for D, D_ in enumerate(Deltas):
    ...
    for m in range(m_l):
        for p in range(p_l):
            for T in range(T_l):
                i, j = p + m*p_l, T
                ax[i, j].plot(ts_, nus_[1, L, m, T, mu, p, D, 0],
                              color=colours[j])
    fig.suptitle(r"$L=%s$, $\mu=%s$, $\Delta=%s$" % (L_, mu_, D_s))
...
for D, D_ in enumerate(Deltas):
    ...
    for m in range(m_l):
        for p in range(p_l):
            for T in range(T_l):
                i, j = p + m*p_l, T
                ax[i, j].plot(ts_, nus_[2, L, m, T, mu, p, D, 1],
                              color=colours[j])
                ax[i, j].relim()
                ax[i, j].autoscale(axis="y")
    fig.suptitle(r"$L=%s$, $\mu=%s$, $\Delta=%s$" % (L_, mu_, D_s))
...
for m, m_ in enumerate(ms):
    for p, pl_ in enumerate(places):
        for D in range(D_l):
            ts_ = ts[D, 0]
            for T in range(T_l):
                ax[T, D].plot(ts_, nus_[0, L, m, T, mu, p, D, 1],
                              color=colours[D])
                ax[T, D].relim()
