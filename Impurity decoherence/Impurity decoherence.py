...
for D, D_ in enumerate(Deltas):
    ...
    for m in range(m_l):
        for p in range(p_l):
            for T in range(T_l):
                i, j = p + m*p_l, T
                ax[i, j].plot(ts_, nus_[2, -1, m, T, 0, p, D, 1],
                              color=colours[j])
                ax[i, j].relim()
                ax[i, j].autoscale(axis="y")
...
for T in range(T_l):
    for m, m_ in enumerate(ms):
        for p, pl_ in enumerate(places):
            ...
            ax[i, j].set_ylim(-1.1 * np.pi, 1.1 * np.pi)
            ax[i, j].set_yticks([-np.pi, 0., np.pi],
                                labels=[r"$-\pi$", "0", r"$\pi$"])
            if j == 0:
                ax[i, j].set_ylabel(r"$\theta(\tau)$ ($m=%s$, %s)" % (m_, pl_))
            else:
...
for D, D_ in enumerate(Deltas):
    ...
    for m in range(m_l):
        for p in range(p_l):
            for T in range(T_l):
                i, j = p + m*p_l, T
                ax[i, j].plot(ts_, nus_[1, -1, m, T, 0, p, D, 0],
                              color=colours[j])
    fig.suptitle(r"$L=%s$, $\mu=%s$, $\Delta=%s$" % (L_, mu_, D_s))
    fig.savefig("m, p, T/ m, p, T θ (L = %s, μ = %s, Δ = %s).pdf" % (L_, mu_,
                                                                     D_))
    for i in range(mp_l):
        for j in range(T_l):
            for line in ax[i, j].get_lines():
...
for T, T_ in enumerate(Ts):
    ...
    for i in range(mp_l):
        for j in range(D_l):
            for line in ax[i, j].get_lines():
                line.remove()
    print("m, p, Δ -log(abs(ν))", T_)

for D in range(D_l):
    ts_ = ts[D, 0]
    ax[0, D].set_xscale("linear")
    ax[0, D].set_xlim(ts_[0], ts_[-1])
    for m, m_ in enumerate(ms):
        for p, pl_ in enumerate(places):
            i, j = p + m*p_l, D
        ...
        for m in range(m_l):
            for p in range(p_l):
                ...
                ax[i, j].plot(ts_, nus_[1, -1, m, T, 0, p, D, 0],
                              color=colours[j])
    fig.suptitle(r"$L=%s$, $T=%s$, $\mu=%s$" % (L_, T_s, mu_))
    fig.savefig("m, p, Δ/ m, p, Δ θ (L = %s, T = %s, μ = %s).pdf" % (L_, T_,
                                                                     mu_))
    for i in range(mp_l):
        for j in range(D_l):
            for line in ax[i, j].get_lines():
...
for D, D_s in enumerate(Delta_strings):
    ...
    for T, T_s in enumerate(T_strings):
        ...
        if T != T_l-1:
            ax[T, D].tick_params(labelbottom=False)
for m, m_ in enumerate(ms):
    for p, pl_ in enumerate(places):
        for D in range(D_l):
            ts_ = ts[D, 1]
            for T in range(T_l):
                ax[T, D].plot(ts_, nus_[2, -1, m, T, 0, p, D, 1],
                              color=colours[D])
                ax[T, D].relim()
...
for m, m_ in enumerate(ms):
    for p, pl_ in enumerate(places):
        for D in range(D_l):
            ts_ = ts[D, 0]
            for T in range(T_l):
                ax[T, D].plot(ts_, nus_[1, -1, m, T, 0, p, D, 0],
                              color=colours[D])
        fig.suptitle(r"$L=%s$, $m=%s$, $\mu=%s$, %s" % (L_, m_, mu_, pl_))
...
del (D, D_, D_s, i, j, L, L_, labels, lengths_half, line, lines, m, m_, mu,
