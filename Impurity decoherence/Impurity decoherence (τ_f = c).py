...
potential μ, impurity location, coupling strength) for linear and logarithmic
time arrays, where the length of the time array is independent of these
parameters.
...
t_nos = 201
t_f = 10000.
ts = np.empty((len(scales), t_nos))
ts[0] = np.linspace(0., t_f, t_nos)
ts[1] = 10.**np.linspace(-1., np.log10(t_f), t_nos)
...
ax[0].set_ylabel(r"$|\nu(\tau)|$")
ax[1].set_xlim(0., t_f)
ax[1].set_xticks(np.linspace(0., t_f, 5))
        ...
        for L, L_ in enumerate(lengths):
            for m, m_ in enumerate(ms):
                for mu, mu_ in enumerate(mus):
                    for p, pl_ in enumerate(places):
                        for i in range(2):
                            ax[i].plot(ts[0], nus_[i, L, m, T, mu, p, D, 0],
                                       color=blinds[i])
                        ax[0].relim()
            ...
            for mu, mu_ in enumerate(mus):
                for p, pl_ in enumerate(places):
                    for T in range(len(Ts)):
                        for i in range(2):
                            ax[i].plot(ts[0], nus_[i, L, m, T, p, D, 0],
                                       color=colours[T])
                    ax[0].relim()
                    ax[0].autoscale(axis="y")
                    fig.suptitle(
...
ax[0, 0].set_ylabel(r"$|\nu(\tau)|$")
ax[1, 0].set_xlim(0., t_f)
ax[1, 0].set_xticks(np.linspace(0., t_f, 5))
            ...
            for p, pl_ in enumerate(places):
                for mu, mu_ in enumerate(mus):
                    for D in range(len(Deltas)):
                        for i in range(3):
                            ax[i].plot(ts[1], nus_[i, L, m, T, mu, p, D, 1],
                                       color=colours[D])
                    for i in range(2):
                        ax[i].relim()
...
del (labels, lines, D, D_, D_s, i, j, L, L_, lengths_half, line, m, m_, mu,
     mu_, nus_, p, p_, pl_, T, t, T_, t_, T_s)
